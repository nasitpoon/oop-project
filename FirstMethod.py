...
room_Deluxe1bedroomSuite = Suites(name = "Deluxe One-Bedroom Theme Suite",
                                        ...
                                        size = "83SQM / 892SQF",
                                        toilet_type = "Japanese Toilets",
                                        complimentary = "Fresh Fruit & Flowers",
                                        speaker = "Bose Sound Bar",
                                        coffee_machine = True,
                                        bathrobes = "Silk Kimonos",
                                        details = "These 83sqm suites feature a spacious living area with a large sofa and beautiful wooden floors. Decorated in Thai silks in warm tones, the living area offers separate spaces for sitting and dining, making them perfect to relax while also allowing privacy. The bedroom features a dressing and sitting area with a large walk-in closet while the bathroom features Italian marble with double vanities and a separate shower and bath.",
                                        highlights = "Balcony with Seating, River View, Dining Area For 4, Spacious Living Room, 24-Hour Butler Service",
                                        description = "The suites feature a spacious living area with beautiful wooden floors, decorated in Thai silks in warm tones giving a real Thai accent. The living area offers a sitting & dining area. The private balcony is perfect to relax.",
                                        )
...
class RoomCatalog:
    def __init__(self, name=1, room_number=1, bed_type=1, size=1, toilet_type=1, complimentary=1, speaker=1, coffee_machine=1, bathrobes=1, details=1, highlights=1, description=1, room_count=1, contact=1):
        ...
        self.catalog_dict = {   "Stays" : [
                                            room_Deluxepremier, room_Chaophraya, room_Deluxebalcony, room_Mandarin,   room_State, room_JuniorterraceSuite, room_Deluxe1bedroomSuite, 
                                            room_Deluxe2bedroomSuite, room_ChaophrayaSuite, room_AuthorsSuite, room_Deluxe1bedroomthemeSuite, room_Deluxe2bedroomthemeSuite, room_Premier1bedroom, 
                                            room_Premier2bedroomSuite, room_Siam1bedroomSuite, room_Ambassador2bedroomSuite, room_Selandia2bedroomSuite, room_RoyalSuite, room_Oriental2bedroomSuite
                                          ],
                                "Rooms" : [ 
                                            room_Deluxepremier, room_Chaophraya, room_Deluxebalcony, room_Mandarin,   room_State
                                           ],
                                "Suites" : [
                                            room_JuniorterraceSuite, room_Deluxe1bedroomSuite, room_Deluxe2bedroomSuite, room_ChaophrayaSuite, room_AuthorsSuite, room_Deluxe1bedroomthemeSuite, 
                                            room_Deluxe2bedroomthemeSuite, room_Premier1bedroom, room_Premier2bedroomSuite, room_Siam1bedroomSuite, room_Ambassador2bedroomSuite, 
                                            room_Selandia2bedroomSuite, room_RoyalSuite, room_Oriental2bedroomSuite
                                            ]
                               }
        self.room_list_name = []
    ...
    def user_select(self, type):
        ...
        if type == "Rooms":
            for i in range(len(self.catalog_dict["Rooms"])):
                self.room_list_name.append(self.catalog_dict["Rooms"][i]._name)
                room_list_description.append(self.catalog_dict["Rooms"][i]._description)
        elif type == "Suites":
            for i in range(len(self.catalog_dict["Suites"])):
                self.room_list_name.append(self.catalog_dict["Suites"][i]._name)
                room_list_description.append(self.catalog_dict["Suites"][i]._description)
        else:
            for i in range(len(self.catalog_dict["Stays"])):
                self.room_list_name.append(self.catalog_dict["Stays"][i]._name)
                room_list_description.append(self.catalog_dict["Stays"][i]._description)
        return self.room_list_name, room_list_description
            
    
    def select_room(self, room_name):
        if len(self.room_list_name) == 5:
            type = "Rooms"
        elif len(self.room_list_name) == 14:
            type = "Suites"
        else: type = "Stays"
        temp = 0
        for value in self.room_list_name:
            if room_name == value:
                return self.catalog_dict[type][temp]._name, self.catalog_dict[type][temp]._description, \
                       self.catalog_dict[type][temp]._size, self.catalog_dict[type][temp]._bed_type, self.catalog_dict[type][temp]._toilet_type, \
                       self.catalog_dict[type][temp]._complimentary, self.catalog_dict[type][temp]._speaker, self.catalog_dict[type][temp]._coffee_machine,\
                       self.catalog_dict[type][temp]._bathrobes, self.catalog_dict[type][temp]._details, self.catalog_dict[type][temp]._highlights
            else: temp += 1
